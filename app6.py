...
def index():
    ...
    return render_template('index6.html', media_urls=media_urls)
...
def upload():
    ...
    try:
        ...
        print(f"media urls are {media_urls}")
        return render_template('index6.html', media_urls=media_urls)  # Pass URLs to template

    except Exception as e:
        return render_template('index6.html', error=f'Failed to load JSON: {str(e)}', media_urls=[]) # Return an empty list if failed

# In extract_media_urls
def extract_media_urls(data: Union[Dict, List, Any]):
    """Recursively extracts image, video, X3D, and URLs from JSON data."""
    global media_urls
