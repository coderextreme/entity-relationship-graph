...
class JsonFileSystemExplorer:
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if isinstance(value, (dict, list)):
                    ...
                    self.tree.insert(node_id, "end", text="Loading...", values=("", "")) # Add "Loading..." so we know to populate on click.
                else:
    ...
    def on_tree_double_click(self, event: tk.Event):
        ...
        if item_type in ("Object", "Array"):
            # Check if it needs to be populated first
            if self.tree.get_children(item_id) == () or self.tree.item(self.tree.get_children(item_id)[0], "text") == "Loading...":
               self.populate_on_demand(item_id, item_text) # Populate before navigating
            else:
    ...
    def populate_on_demand(self, item_id, item_text):
        """Populates the tree when the user attempts to expand a node."""
        new_path = os.path.join(self.current_path, item_text)
